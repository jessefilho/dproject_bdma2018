...
CREATE SEQUENCE public.dim_song_idsong_seq_2;

CREATE TABLE public.dim_song (
                idsong INTEGER NOT NULL DEFAULT nextval('public.dim_song_idsong_seq_2'),
                CONSTRAINT dim_song_pk PRIMARY KEY (idsong)
);


ALTER SEQUENCE public.dim_song_idsong_seq_2 OWNED BY public.dim_song.idsong;

CREATE SEQUENCE public.dim_popularitydate_idpopularitydate_seq_1;

CREATE TABLE public.dim_popularitydate (
                idpopularitydate INTEGER NOT NULL DEFAULT nextval('public.dim_popularitydate_idpopularitydate_seq_1'),
                popularitymonthYear VARCHAR NOT NULL,
                popularitymonth VARCHAR NOT NULL,
                popularityYear VARCHAR NOT NULL,
                CONSTRAINT dim_popularitydate_pk PRIMARY KEY (idpopularitydate)
);


ALTER SEQUENCE public.dim_popularitydate_idpopularitydate_seq_1 OWNED BY public.dim_popularitydate.idpopularitydate;

CREATE TABLE public.fct_popularitysong (
                idsong INTEGER NOT NULL,
                idpopularitydate INTEGER NOT NULL,
                popularitySong VARCHAR NOT NULL,
                CONSTRAINT fct_popularitysong_pk PRIMARY KEY (idsong, idpopularitydate)
);
...
CREATE TABLE public.fct_popularityartist (
                ...
                idpopularitydate INTEGER NOT NULL,
                popularityArtist REAL NOT NULL,
                CONSTRAINT fct_popularityartist_pk PRIMARY KEY (idartistgroup, idpopularitydate)
);
...
CREATE TABLE public.fct_song (
                ...
                CONSTRAINT fct_song_pk PRIMARY KEY (idreleasedatesong, idgenre, idartistgroup, idmember, idreleaselocationsong, idcover, idalbum, idlabels, idsong)
);
...
ALTER TABLE public.fct_song ADD CONSTRAINT dim_song_fct_song_fk
FOREIGN KEY (idsong)
REFERENCES public.dim_song (idsong)
ON DELETE NO ACTION
ON UPDATE NO ACTION
NOT DEFERRABLE;
...
ALTER TABLE public.fct_popularityartist ADD CONSTRAINT dim_popularitydate_fct_popularityartist_fk
FOREIGN KEY (idpopularitydate)
REFERENCES public.dim_popularitydate (idpopularitydate)
ON DELETE NO ACTION
ON UPDATE NO ACTION
NOT DEFERRABLE;
